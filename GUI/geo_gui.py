...
#					 selectcolor=dim)
	...
	def plot(self, canvas, ax):
		...
		try: #Count: 14
			...
			self.face2.config(state=tk.DISABLED)
			self.f2.config(bg=dim, fg=dim, relief=tk.RIDGE)
			self.face3.config(state=tk.DISABLED)
			self.f3.config(bg=dim, fg=dim, relief=tk.RIDGE)
			self.raa_entry.config(state=tk.DISABLED,bg=dim, fg=dim, activebackground=dim, highlightthickness=0, troughcolor=dim)
			...
			try: # Count: 12
				...
				self.face3.config(state=tk.DISABLED)
				self.f3.config(bg=dim, fg=dim, relief=tk.RIDGE)
				self.si_entry.config(state=tk.ACTIVE, bg=dim, fg=dimf, activebackground=dim, highlightthickness=0, troughcolor=dimt)
				self.ed_entry.config(state=tk.ACTIVE, bg=dim, fg=dimf, activebackground=dim, highlightthickness=0, troughcolor=dimt)
				self.pi_entry.config(state=tk.ACTIVE, bg=dim, fg=dimf, activebackground=dim, highlightthickness=0, troughcolor=dimt)
				self.raa_entry.config(state=tk.ACTIVE, bg=dim, fg=dimf, activebackground=dim, highlightthickness=0, troughcolor=dimt)
				self.ram_entry.config(state=tk.ACTIVE, bg=dim, fg=dimf, activebackground=dim, highlightthickness=0, troughcolor=dimt)
				self.h_entry.config(state=tk.ACTIVE, bg=dim, fg=dimf, activebackground=dim, highlightthickness=0, troughcolor=dimt)
			except TypeError:
				try: # Count: 11
					...
					try: # Count: 10
						...
						try: # Count: 9
							...
							try: # Count: 8
								...
								self.face3.config(state=tk.DISABLED)
								self.face2.config(state=tk.ACTIVE)
								self.f3.config(bg=dim, fg=dim, relief=tk.RIDGE)
								...
								try:
									...
									self.face2.config(state=tk.DISABLED)
									self.f2.config(bg=dim, fg=dim, relief=tk.RIDGE)
									self.face3.config(bg=dim, fg=dim, state=tk.DISABLED)
									self.f3.config(bg=dim, fg=dim, relief=tk.RIDGE)
									self.si_entry.config(state=tk.ACTIVE, bg=dim, fg=dimf, activebackground=dim, highlightthickness=0, troughcolor=dimt)
									self.ed_entry.config(state=tk.ACTIVE, bg=dim, fg=dimf, activebackground=dim, highlightthickness=0, troughcolor=dimt)
									self.pi_entry.config(state=tk.DISABLED, bg=dim, fg=dim, activebackground=dim, highlightthickness=0, troughcolor=dim)
								...
								try: # Count: 8
									...
									self.face2.config(state=tk.ACTIVE)
									self.face3.config(state=tk.ACTIVE)
									self.f2.config(relief=tk.GROOVE)
									...
									self.raa_entry.config(state=tk.DISABLED, bg=dim, fg=dim, activebackground=dim, highlightthickness=0, troughcolor=dim)
										...
										try: # Count:7
											...
											self.f3.config(bg=dim, fg=dim, relief=tk.RIDGE)
											self.ram_entry.config(state=tk.ACTIVE, bg=dim, fg=dimf, activebackground=dim, highlightthickness=0, troughcolor=dimt)
											self.pi_entry.config(state=tk.DISABLED, bg=dim, fg=dim, activebackgroun=dim, highlightthickness=0, troughcolor=dim)
											self.si_entry.config(state=tk.DISABLED,bg=dim, fg=dim, activebackground=dim, highlightthickness=0, troughcolor=dim)
											self.ed_entry.config(state=tk.DISABLED, bg=dim, fg=dim, activebackground=dim, highlightthickness=0, troughcolor=dim)
										except TypeError:
											try: #Count: 7
