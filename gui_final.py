...
class Geometry(tk.Frame):
    ...
    def createWidgets(self):
        ...
        self.axis_zoom = tk.Scale(root, from_=1, to=100, width=40, orient=tk.HORIZONTAL, variable=self.axis_limits, command=lim)
    ...
    def test(self, canvas, ax):
        ...
        s[self.shape_set.get()].set_alpha(self.a_entry.get())  # Transparency of figure
        s[self.shape_set.get()].set_edgecolor(self.ec_entry.get(self.ec_entry.curselection()[0]))  # Edge color of the lines on the figure
        s[self.shape_set.get()].set_linewidth(self.ew_entry.get())  # Line width of the edges
        s[self.shape_set.get()].set_facecolor(self.c_entry.get(self.c_entry.curselection()[0]))  # General color of the figure
